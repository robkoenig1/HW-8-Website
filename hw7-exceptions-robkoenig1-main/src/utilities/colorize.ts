...
/**
 * Logs the given text to the console with color, obeying commands given in square brackets.
 * @param text Any number of objects that can be converted to strings.
 */
export function colorLog(...text: toStringable[]) {
    console.log(text.map((t) => colorize(t.toString())).join(""));
}